...
import { getFirestore, collection, doc, setDoc, addDoc, updateDoc, deleteDoc, onSnapshot, getDoc, serverTimestamp } from "https://www.gstatic.com/firebasejs/12.18.0/firebase-firestore.js";
...
function msg(el,text,ok=false){el.textContent=text;el.classList.toggle("ok",ok);}

$("#loginForm").addEventListener("submit",async e=>{
  e.preventDefault(); msg($("#loginMessage"),"Signing in…");
  try{await signInWithEmailAndPassword(auth,$("#adminEmail").value.trim(),$("#adminPassword").value);msg($("#loginMessage"),"");}
  catch(err){msg($("#loginMessage"),"Login failed: "+(err.message||err.code));}
});
...
onAuthStateChanged(auth,user=>{
  ...
  if(user) $("#adminIdentity").textContent=user.email||"Authenticated admin";
});
...
function payload(){
  ...
  return {
    name:$("#pName").value.trim(), nameMM:$("#pNameMM").value.trim(),
    price:Number($("#pPrice").value||0), category:$("#pCategory").value,
    colorKey:$("#pColorKey").value, stockStatus:stock,
    waitingPeriod:stock==="preorder"?($("#pWaiting").value.trim()||"2 weeks"):"",
    badge:$("#pBadge").value||(stock==="preorder"?"Pre-order":"In Stock"),
    powers:powersInput(), imageUrl:$("#pImage").value.trim(),
    description:$("#pDescription").value.trim(), descriptionMM:$("#pDescriptionMM").value.trim(),
    updatedAt:serverTimestamp()
  };
}
function resetForm(){
  $("#productForm").reset();$("#editId").value="";$("#formTitle").textContent="Add product";
  $("#saveBtn").textContent="Save product";$("#pStock").value="instock";$("#pBadge").value="In Stock";
  $("#pWaiting").value="2 weeks";msg($("#formMessage"),"");
}
$("#resetBtn").addEventListener("click",resetForm);
$("#pStock").addEventListener("change",()=>{
  if($("#pStock").value==="preorder"){$("#pBadge").value="Pre-order";if(!$("#pWaiting").value)$("#pWaiting").value="2 weeks";}
  else if($("#pBadge").value==="Pre-order")$("#pBadge").value="In Stock";
});

$("#productForm").addEventListener("submit",async e=>{
  e.preventDefault();const p=payload();if(!p.name)return msg($("#formMessage"),"Product name is required.");
  $("#saveBtn").disabled=true;msg($("#formMessage"),"Saving…");
  try{
    const id=$("#editId").value;
    if(id){await updateDoc(doc(db,"products",id),p);msg($("#formMessage"),"Product updated.",true);}
    else{await addDoc(collection(db,"products"),{...p,createdAt:serverTimestamp()});msg($("#formMessage"),"Product added.",true);}
    setTimeout(resetForm,700);
  }catch(err){msg($("#formMessage"),"Save failed: "+(err.message||err.code));}
...
onSnapshot(collection(db,"products"),snap=>{
  ...
  if(snap.empty){box.innerHTML='<p class="muted">No Firebase products yet.</p>';return;}
  const rows=snap.docs.map(d=>({id:d.id,...d.data()})).sort((a,b)=>(a.name||"").localeCompare(b.name||""));
  box.innerHTML=rows.map(p=>`
    <article class="admin-product">
      <div class="admin-thumb">${p.imageUrl?`<img src="${esc(p.imageUrl)}" alt="">`:`<span>${esc((p.name||"?")[0])}</span>`}</div>
      <div class="admin-product-info"><strong>${esc(p.name)}</strong><span>AED ${Number(p.price||0).toFixed(0)} • ${p.stockStatus==="preorder"?"Pre-order":"In Stock"} • ${esc(p.colorKey||"")}</span><small>${esc(p.category||"")}</small></div>
      <div class="admin-actions"><button class="edit-btn" data-id="${p.id}">Edit</button><button class="delete-btn" data-id="${p.id}">Delete</button></div>
    </article>`).join("");
  box.querySelectorAll(".edit-btn").forEach(b=>b.addEventListener("click",()=>editProduct(b.dataset.id)));
  box.querySelectorAll(".delete-btn").forEach(b=>b.addEventListener("click",()=>removeProduct(b.dataset.id)));
},err=>{$("#productList").innerHTML='<p class="message">Could not read Firestore: '+esc(err.message)+'</p>';});

async function editProduct(id){
  const s=await getDoc(doc(db,"products",id));if(!s.exists())return;const p=s.data();
  $("#editId").value=id;$("#pName").value=p.name||"";$("#pNameMM").value=p.nameMM||"";
  $("#pPrice").value=p.price??0;$("#pCategory").value=p.category||"Contact Lenses";$("#pColorKey").value=p.colorKey||"gray";
  $("#pStock").value=p.stockStatus==="preorder"?"preorder":"instock";$("#pWaiting").value=p.waitingPeriod||"2 weeks";
  $("#pBadge").value=p.badge||($("#pStock").value==="preorder"?"Pre-order":"In Stock");
  $("#pPowers").value=Array.isArray(p.powers)?p.powers.join(", "):(p.powers||"");
  $("#pImage").value=p.imageUrl||p.image||"";$("#pDescription").value=p.description||p.desc||"";
  $("#pDescriptionMM").value=p.descriptionMM||p.descMM||"";$("#formTitle").textContent="Edit product";$("#saveBtn").textContent="Update product";
  window.scrollTo({top:0,behavior:"smooth"});
}
async function removeProduct(id){
  if(!confirm("Delete this product?"))return;
  try{await deleteDoc(doc(db,"products",id));}catch(err){alert("Delete failed: "+(err.message||err.code));}
}

$("#importBtn").addEventListener("click",async()=>{
  if(!confirm("Import the 15 products from your current website into Firestore?"))return;
  const btn=$("#importBtn");btn.disabled=true;btn.textContent="Importing…";let added=0,skipped=0;
  try{
    for(const p of oldProducts){
      const id="legacy-"+String(p.id);const ref=doc(db,"products",id);const existing=await getDoc(ref);
      if(existing.exists()){skipped++;continue;}
      await setDoc(ref,{
        name:p.name||"",nameMM:p.nameMM||"",price:Number(p.price||0),category:p.category||"Contact Lenses",
        colorKey:p.colorKey||"clear",stockStatus:p.stockStatus==="preorder"?"preorder":"instock",
        waitingPeriod:p.stockStatus==="preorder"?"2 weeks":"",badge:p.badge||"",powers:p.powers??null,
        imageUrl:p.image||"",description:p.desc||"",descriptionMM:p.descMM||"",
        createdAt:serverTimestamp(),updatedAt:serverTimestamp()
      });added++;
    }
    alert(`Import finished. Added ${added}, skipped ${skipped}.`);
  }catch(err){alert("Import failed: "+(err.message||err.code));}
  finally{btn.disabled=false;btn.textContent="Import current 15 products";}
});
resetForm();
